...
char serialib::openDevice(const char *Device, const unsigned int Bauds,
    ...
    case 1000000 :   dcbSerialParams.BaudRate=1000000; break;
    case 2000000 :   dcbSerialParams.BaudRate=2000000; break;
    default : return -4;
    }
    //select data size
    BYTE bytesize = 0;
...
int serialib::readStringNoTimeOut(char *receivedString,string finalChars,unsigned int maxNbBytes)
{
    ...
    while (NbBytes<maxNbBytes)
    {
        ...
        if (charRead==1)
        {
            ...
            if (receivedString[NbBytes]==10 && lineEndFlag)
            {
                printf("finish\n");
...
int serialib::readString(char *receivedString,char finalChar,unsigned int maxNbBytes,unsigned int timeOut_ms)
{
    ...
    if (timeOut_ms==0) return 0;//readStringNoTimeOut(receivedString,finalChar,maxNbBytes);
